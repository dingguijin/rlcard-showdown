...
function PvEDoudizhuDemoView() {
    ...
    const proceedNextTurn = async (playingCard, rankOnly = true) => {
        ...
        if ((gameState.currentPlayer + 1) % 2 === mainPlayerId) {
            hintIdx = -1;
        ...
        // DGJ: peasant win
        const is_p_role = playerInfo[gameState.currentPlayer].role === "peasant";
        if (newHand.length === 1 && is_p_role) {
            setGameStatus('over');
            syncGameStatus = 'over';
            is_game_over = true;
        }
        ...
        if (is_game_over) {
            ...
            setTimeout(() => {
                gameEndDialogTitle = winner.role === 'peasant' ? t('doudizhu.peasants_win') : t('doudizhu.landlord_win');
                setStatisticRows([
                    {
    ...
    const requestApiPlay = async () => {
        ...
        const num_cards_left_landlord = gameState.hands[playerInfo.find((player) => player.douzeroPlayerPosition === 0).index].length;

        // DGJ: FIXME: -1
        const num_cards_left_landlord_down = gameState.hands[playerInfo.find((player) => player.douzeroPlayerPosition === 1).index].length;
        
        //DGJ: always set to 999999999
        const num_cards_left_landlord_up = 9;
    
        const three_landlord_cards = cardArr2DouzeroFormat(threeLandlordCards.slice().reverse());
        const card_play_action_seq = gameHistory.map((cards) => {
            return cardArr2DouzeroFormat(cards);
        }).join(',');

        //DGJ: keep hands has 3 elements
        ...
        //DGJ:
        const played_cards_landlord_up = "";
    ...
    const handleSelectRole = (role) => {
        ...
        playerInfo[(landlordIdx + 1) % 2].douzeroPlayerPosition = 1;
    ...
    const handleCloseGameEndDialog = () => {
        ...
        initHands = [
            shuffledDoudizhuDeck.slice(3, 20),
            shuffledDoudizhuDeck.slice(20, 37),
            shuffledDoudizhuDeck.slice(37, 46)            
        ];
                        ...
                        <Paper className={'doudizhu-probability-paper'} elevation={3}>
                            ...
                            <div className={'probability-player'} style={{ height: '19px', textAlign: 'center' }}>
                                {playerInfo.length > 0 && gameState.currentPlayer !== null ? (
                                    <span>
                                        {t(
                                            `doudizhu.${['landlord', 'landlord_down', 'landlord_up'][
